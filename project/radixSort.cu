...
// Step 1 - split bits and create  intermediate array of all bits in 0th, 1th ,2nd--9th position
// Step 2 - Prefix sum of intermediate array
// Step 3 - Move element at desired location depending on its bit value

// this function performs the prefix scan of the bit array which is then used to determine the actual
// position where the element should be positioned in the array.
...
// this function extracts the individual bit, and repositions the element to the correct position depending
// on the bit value
...
__global__ void GPU_radix_sort(int *d_input,int *d_output,int size) {
    ...
    // NOT TRUE WITH NUMBERS BIGGER THAN 999
    // max number is 999 and this can be represented using 10 bits, so for loop will iterate from bit =0 to bit =9. 
